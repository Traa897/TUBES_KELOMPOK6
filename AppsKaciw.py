...
class BestSellerRecommendationPage(QMainWindow):
    ...
    class FeedbackDialog(QDialog):
        def __init__(self, item_name, parent=None):
            super().__init__(parent)
            self.setWindowTitle('Beri Penilaian')
        ...
        # Item name label
            item_label = QLabel(f'Penilaian untuk {item_name}')
            item_label.setStyleSheet('font-weight: bold; font-size: 18px;')
        ...
        # Rating section
            ...
            self.comment_input.setPlaceholderText('Tulis komentar Anda tentang makanan ini...')
            layout.addWidget(comment_label)
            layout.addWidget(self.comment_input)

            # Submit button
            submit_btn = QPushButton('Kirim Penilaian')
        ...
        def submit_feedback(self):
            # Get selected rating (convert to numeric value)
            self.rating = self.rating_combo.currentIndex() + 1
            ...
            if not self.comment.strip():
                ...
                QMessageBox.information(self, 'Terima Kasih', 'Penilaian Anda telah dikirim')
                self.accept()

# Modify the CartDialog class to include feedback functionality
class CartDialog(QDialog):
    def __init__(self, cart_items, parent=None):
        # ... (existing __init__ code remains the same)
        
        # Add a new attribute to track ordered items
        self.ordered_items = []
    ...
    def process_order(self):
        ...
        if reply == QMessageBox.Yes:
            # Store the items before clearing the cart
            self.ordered_items = self.cart_items.copy()
            
            QMessageBox.information(self, "Pesanan Berhasil", "Terima kasih telah memesan!")
            
            # Open feedback dialogs for each ordered item
            for item in self.ordered_items:
                feedback_dialog = FeedbackDialog(item.name, self)
                feedback_dialog.exec_()
            
            # Clear the cart after feedback
            self.cart_items.clear()
            self.update_cart()
            self.accept()

# Optional: Add a method to view past feedbacks (you can expand on this)
class FeedbackManager:
    def __init__(self, feedback_file="feedback_data.txt"):
        self.feedback_file = feedback_file

    def save_feedback(self, item_name, rating, comment):
        with open(self.feedback_file, "a", encoding="utf-8") as file:
            file.write(f"{item_name},{rating},{comment}\n")

    def get_feedbacks_for_item(self, item_name):
        feedbacks = []
        try:
            with open(self.feedback_file, "r", encoding="utf-8") as file:
                for line in file:
                    parts = line.strip().split(',')
                    if parts[0] == item_name:
                        feedbacks.append({
                            'rating': int(parts[1]),
                            'comment': parts[2]
                        })
